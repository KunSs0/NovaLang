...
`RuntimeWorkspace` 按完整模块图生成一份联合编译输入和一份字节码产物，不按入口重复编译依赖闭包。被多个入口引用的模块只在顶层出现一次；仅由单个入口使用的模块放入该入口独立的对象命名空间，使多个 YAML 虚拟入口仍可声明同名 `execute` 等函数。

联合产物使用整个源码图作为缓存键。缓存命中时不触发编译；上层清空缓存后，一次 Workspace 加载也只重新编译整张模块图一次。任意模块编译失败时整个 Workspace 加载失败，不保留逐入口编译的兼容路径。
