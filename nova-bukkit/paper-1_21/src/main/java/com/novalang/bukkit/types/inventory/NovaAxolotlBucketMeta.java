...
public final class NovaAxolotlBucketMeta {
    private static final String TYPE = "org.bukkit.inventory.meta.AxolotlBucketMeta";
    private static final String VARIANT = "org.bukkit.entity.Axolotl$Variant";

    private NovaAxolotlBucketMeta() { }

    public static void register(JavaTypes.Builder builder) {
        Class<?> type = PaperInventoryReflection.type(NovaAxolotlBucketMeta.class, TYPE);
        Class<?> variant = PaperInventoryReflection.type(NovaAxolotlBucketMeta.class, VARIANT);
        Method set = PaperInventoryReflection.method(type, "setVariant", variant);
        Method has = PaperInventoryReflection.method(type, "hasVariant");
        Method clone = PaperInventoryReflection.method(type, "clone");
        builder.extension(type, "setVariant", f -> f.param("variant", variant).returns(Void.TYPE).invoke(a -> PaperInventoryReflection.invoke(set, a[0], a[1])));
        builder.extension(type, "setVariant", f -> f.param("variant", String.class).returns(Void.TYPE).invoke(a -> { Object value = enumValue(variant, (String) a[1]); if (value != null) { PaperInventoryReflection.invoke(set, a[0], value); } return null; }));
